...
- A user title write must persist its `chatKey -> cliSessionId/filePath` binding in the same SQLite transaction when provider identity already exists. An unbound Claude or Codex draft may persist its user title by `chatKey` first; the first runtime binding attaches later without replacing `title/titleSource`. Startup may backfill only explicit legacy user renames from panel state; scan-derived panel titles are never authoritative.
...
- Claude session ownership is `one chatKey -> one active transcript head`. Before every send, main resolves that head from SQLite and overrides renderer/in-memory UUID caches. Normal compact/resume keeps the same Claude `session_id`; an SDK-returned different `session_id` or `No conversation found` must fail closed instead of silently starting a contextless transcript. Historical duplicate runtime bindings keep their JSONL files but noncanonical bindings are marked `detached=true, resume_allowed=false`; scans and panel persistence expose only the active head and dedupe by stable `chatKey`. When an old dev/profile database still assigns that transcript exclusively to an unrenamed `claude::scan-*` placeholder, the first stable runtime write may atomically transfer the bindings to the real `chatKey`; user-owned or non-scan owners must never be stolen.
