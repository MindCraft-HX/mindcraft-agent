...
- Claude：`model/effort` 优先使用当前 tab/session 值；已绑定会话写入 `<cliSessionId>.meta.json`；新建 draft 会话首轮由主进程在拿到真实 `cliSessionId` 后延迟写入。
- Claude：`scanCliSessionsForProject()` 扫描 `.jsonl` 时同步读取同名 `.meta.json`，delete chat 同步删除 sidecar。
- CodeX：本地 `@openai/codex-sdk@0.135.0` 确认 `startThread/resumeThread(options)` 支持 `model` 与 `modelReasoningEffort`；UI 已改为 tab 级模型状态，不再把 `/model` 同步到所有历史 tab。
- 风险控制：未引入中央索引文件；close-tab 仍只影响 panel state；磁盘 artifact 继续以 `cliSessionId/filePath` 定位。
- 边界：T089 不等于 T091。当前已支持每个 session/tab 独立模型；但运行中换模型仍是关闭旧 query 后按新模型 resume/recreate，未实现 SDK 原生 `query.setModel()` 的不中断切换。

## 6. review 验证清单

- [x] close-tab 不删 `.meta.json`（设计保持：close 只影响 panel state）
- [x] re-open project 恢复模型选择
- [x] delete chat 同步删 `.meta.json`
- [x] 新 session 用全局默认值初始化 meta（draft 首轮延迟到 `cliSessionId` 可用后写入）
- [x] SelectModel 打开时显示当前 session 的模型（而非全局）
- [x] 改模型后 query 用新模型
- [x] 不传 model 时 fallback 全局（向后兼容老 session）
- [x] 人工回归通过：Claude 使用识图/非识图模型验证 per-session 生效；CodeX 因候选模型均可识图，按 tab 状态、持久化与 SDK per-thread 参数链路验证。
