...
4. 若答案已完成但随后十分钟超时，检查外部 CLI 的 stdout/子进程树是否在
   `terminal_seen` 后仍未关闭；此时只能在短 grace 后关闭该 run 的 owned tree
5. renderer 收到 error 或 `session_close_timeout` 时只能停止可见 thinking，必须保持
   `_awaitingDone`，禁止定时重试；排队消息等待匹配的 `agent_done`
...
| 2026-07-18 | Codex external CLI image resume | `exec --image <FILE>...` 是可变参数，但 transport 把 `resume <thread-id>` 放在图片参数之后 | CLI 将 `resume` 和 UUID 当成图片路径，已有会话带图发送时静默新建 thread；扫描再将新 transcript 识别为 external session，阻塞式确认框进一步干扰输入 | `resume <thread-id>` 必须排在所有 `--image` 之前；resume 返回不同 thread id 时失败并保留原绑定；发送即显式 claim，禁止在 composer 热路径使用 `window.confirm`；覆盖带图 resume、claim 失败保留草稿和 thread ownership 回归测试 |
| 2026-07-19 | Codex ownership guard + SQLite binding source | `9fbebf3` 将 scan chat 限制为仅 `source=user` 可注册，但运行时新建 thread 写成 `source=runtime`；同时 restore/rename/scan 对 `resumeAllowed` 与 ownership 的解释不一致 | 首次映射丢失后走 `startThread`；新 thread 完成后注册又被删除，后续每轮都创建无上下文 transcript，生产列表重复、dev 显示碎片 | 单 chat 单 thread；`scan` 只读、`user/runtime` owned；重命名保留 binding source；主进程发送前从 SQLite 恢复 owned binding；不同 `thread.started` 视为 resume 失败；repository 拒绝第二个 distinct owned thread；禁止聚合 transcript |
| 2026-07-19 | Codex ownership guard blocked historically corrupted chats | 单 thread guard 能阻止新污染，但旧 chat 已经含有一个 canonical binding 和多个历史 `runtime` fragment binding | 原会话内容仍完整，但发送前 claim 被 `different provider thread` 拒绝，用户无法继续旧会话 | 仅当目标就是确定的 canonical owned thread 时，事务内移除冲突的历史 `runtime` bindings；`user` 冲突继续 fail closed；不删除 provider transcript；claim 成功立即持久化 SQLite |
| 2026-07-19 | Claude SDK `0.2.141` -> `0.3.214` lifecycle gap | 新版会持久化后台任务；旧 Query 在 `TaskOutput` 等待中被 provider reset/崩溃关闭后，普通 resume 可能选回旧工具分支，产生 synthetic `No response requested.`，随后真实用户消息写入 transcript 但不进入模型 | 消息 `0.0s` 结束；首次 checkpoint 恢复成功后，下一轮普通 resume 仍可能重新选错旧分支 | 保持同一 `cliSessionId`；沿 transcript 当前尾部的 `parentUuid` 主链回溯，以最近完整 assistant 文本作为 `resumeSessionAt`；文件顺序不能代替分支顺序；禁止 fork/合并/写修复行；`background_tasks_changed` 驱动当前会话后台任务指示，旧版 edge 事件兜底 |
| 2026-07-20 | Claude stable `chatKey` accumulated multiple transcript heads | Claude runtime writes did not validate thread ownership; send trusted the in-memory UUID map; `msg.uuid` (message UUID) and a newest-file fallback could be mistaken for provider session identity; `No conversation found` retried without resume. A compact summary then made the contextless branch look superficially continuous. Dev/profile crossing could also leave the real transcript owned only by an ephemeral `claude::scan-*` row. | One logical chat appeared several times with independent JSONL files; restarting or rescanning changed which branch was registered; later branches began with the app-carried compact summary; replies could finish and then show `Provider key already belongs to claude::scan-*`. | Resolve the active Claude head from SQLite before send; accept only SDK `session_id`; reject resume rollover and `No conversation found` without creating a branch; remove newest-file adoption; keep superseded JSONL untouched but detach their bindings; scan and panel state dedupe by stable `chatKey`; atomically transfer only unrenamed scan-placeholder ownership to the stable chat. Codex ownership rules remain unchanged. |
| 2026-07-20 | Codex logical completion left external CLI transport open | `turn.completed` finished rendering, but the CLI adapter still waited for stdout EOF and child exit. Descendants spawned by commands such as Electron E2E could inherit handles; the ten-minute idle watchdog then misreported a completed turn as timeout. Renderer error handling separately cleared the lock and retried `session_close_timeout` every 1.2 seconds, resetting the footer timer. | A completed answer later showed timeout; subsequent sends collided with the still-owned run; the footer repeatedly returned to `运行中 0s` until restart. | Keep logical terminal and transport closure separate; replace the idle timeout with a five-second post-terminal grace; then terminate only that run's owned process tree and preserve completed/failed reason. Renderer terminal errors keep `_awaitingDone`; rejected input queues without polling and flushes only on matching `agent_done`. |
| 2026-07-21 | Claude manual abort resumed into an empty turn or invalid checkpoint | Resume recovery treated thinking-only and `text + stop_reason: tool_use` rows as safe, then used the latest JSONL row rather than the active parent chain; a completed assistant row after async Agent launch was also unsafe because SDK first replayed stopped-task notifications and consumed the human input. Abort simultaneously deleted the `chatKey` entry before the old Query finalizer exited. | `0.1s` empty assistant shell, transcript continued while APP stopped rendering, `No message found with message.uuid` for a UUID visibly present in an abandoned branch, or synthetic/stopped-task notifications appeared without a real answer. | Walk the latest graph head's `parentUuid` chain; track async launch IDs and task-notification completion, and roll back before the earliest unfinished launch. Each Query owns a `runId`; abort retains ownership and renderer lock until the owned finalizer releases it. A real SDK `forkSession` smoke test against the affected transcript returned `RECOVERY_OK/success` from the selected pre-launch checkpoint without modifying the original session. |
| 2026-07-22 | Claude interactive requests were renderer-completed before SDK acknowledgement | `AskUserQuestion`, permission, and plan resolvers lived in an unowned global map. The renderer hid prompts on deactivation, fabricated first-option answers on close/timeout, and marked cards done before the response IPC proved that the SDK permission stream was still open. | A question stopped being prominent after a tab switch; selecting an answer made the card look complete while Claude never continued. The transcript recorded `Tool permission stream closed before response received`. | Interactive requests are owned by `chatKey + runId + requestId + sender`; run abort/replacement/finalization cancels only its requests. Response IPC returns `{ ok }`, and Ask cards complete only after `{ ok: true }`. Closing only minimizes, active live prompts restore on activation, and timeout never fabricates an answer. |
| 2026-07-22 | Claude next input collided with post-result background cleanup | A Query remained alive after its main `result` while Claude Code reconciled a background Bash task. The app sent the next message through `streamInput()` because the Query still existed. On resume, Claude Code 2.1.214 first emitted `origin.kind=task-notification, num_turns=0, result=''`, then processed the real user turn; the app treated that cleanup result as terminal and released the Query before the real answer. | The human message was persisted in the same transcript but had no assistant child; the APP showed a `0.1s` empty turn and unlocked. A temporary-config A/B against the affected transcript reproduced the cleanup result followed by a normal one-turn answer. | Store `resultReceived` on the owned run and allow `streamInput()` only before the main result. Replace a trailing Query for the next input, keep the same provider session, preserve the latest completed answer after a background command, and ignore only the strict zero-turn task-notification cleanup result. Async Agent recovery remains conservative. No transcript fork or merge is introduced. |
| 2026-07-23 | Claude recovery overrode a normally completed SDK leaf | Recovery treated any historically unresolved async Agent as active and pinned every fresh resume, even after the SDK wrote a later completed answer and `last-prompt.leafUuid`. | A short follow-up such as "fix it" resumed from an older answer, lost the immediately preceding review, and showed a sharp cache drop. | Treat a trailing valid completed leaf marker as authoritative and leave normal resume unpinned; ignore stale markers followed by newer UUID-bearing entries and expand tail reads when the marked leaf is not loaded. |
| 2026-07-23 | Claude SDK `0.2.141` -> `0.3.214` exposed an incomplete Plan integration | The pre-upgrade prompt-driven Plan flow had worked in practice despite using `permissionMode: default` and an old bisect-disabled `PlanReviewDialog`. The 2026-07-18 SDK upgrade was not accompanied by Plan adaptation; Claude Code 2.1.214 can emit `ExitPlanMode.input = {}` instead of the older `{ plan, planFilePath }`, while runtime mode switching still changed only an app string. | A previously usable Plan workflow became unreliable: Claude could enter Plan mode, print a plan, then finish without a usable approval flow; when ExitPlanMode arrived, only a pending tool card could remain. | Map app run mode to native SDK `permissionMode: plan`, call `Query.setPermissionMode()` for a live turn, and own PlanReview by `chatKey + runId + requestId`. Reuse the streamed ExitPlanMode card, preview the preceding assistant plan for new CLI rows, wait for response IPC acknowledgement, and restore live review state after tab activation. |
| 2026-07-23 | Claude `AskUserQuestion` IPC raced its streamed tool row | When the ordinary assistant stream arrived first, it created a `running` card without `requestId`. The interactive IPC then built the request-owned `pending` card, but generic adjacent-message dedup discarded it as the same tool/text; deferred `nextTick()` lookup still found the non-interactive card. | The inline card showed `等待回答中` and `运行中` but had no answer button or dialog. About 55 seconds later Claude Code closed the unanswered permission stream; the transcript recorded `Tool permission stream closed before response received`, and subsequent `ExitPlanMode` attempts failed with `Stream closed`. | Carry SDK `toolUseID` on the interactive IPC, merge both arrival orders into one card by `toolUseId`, and pass that exact request-owned message directly to the dialog instead of re-querying the message tail. |
| 2026-07-27 | Claude abort left an interactive control stream permanently closed | A manual abort while `AskUserQuestion` was waiting closed the SDK permission stream. The app retained the same live Query and sent later input through `streamInput()`, but every later `AskUserQuestion`, permission prompt, and `ExitPlanMode` then failed immediately with `AbortError: Stream closed`. The Ask dialog also offered no explicit non-answer path, making abort the only way to leave an unwanted question. | A question card became `错误` while still saying `等待回答中`; later interactive tools failed in milliseconds, although ordinary read-only tools and text output could continue. | Ask dialogs provide `暂不回答`, acknowledged by the main process as a normal deny result rather than a fabricated answer. A `Tool permission request failed ... stream closed` tool result marks the owned Query's control stream broken, cancels its live requests, and forces the next input to create a fresh Query while resuming the same `cliSessionId`. |
| 2026-07-23 | Model-entered native Plan mode was not reflected in app state | A turn could start in SDK `default` mode and then call `EnterPlanMode`; the SDK switched modes internally, but the app selector retained its previous `runMode`. | The transcript was read-only and could ask planning questions while the toolbar still showed Edit automatically, making later input and final approval behavior ambiguous. | Consume the SDK-authoritative `system/init` and `system/status` `permissionMode: plan` fields and persist `tab.runMode = plan_mode`. Do not reverse-map `default`, because SDK `default` represents both app Ask-before-edits and Edit-automatically modes. |
