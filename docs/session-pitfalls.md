...
➡️ 详见 `docs/bugs/claude-session-duplicate-split.md` 根因 B
...
➡️ 详见 `docs/bugs/claude-session-duplicate-split.md` 根因 A
➡️ 详见 `docs/bugs/codex-conversation-interruption.md` 第二轮根因
...
➡️ 详见 `docs/bugs/claude-session-duplicate-split.md` 根因 E
➡️ 详见 `docs/bugs/codex-conversation-interruption.md` R1 路径 A
...
➡️ 详见 `docs/bugs/claude-session-duplicate-split.md` 根因 C, D, E
➡️ 详见 `docs/bugs/codex-conversation-interruption.md` R1 路径 B
...
➡️ 详见 `docs/bugs/codex-conversation-interruption.md` R1
...
➡️ 详见 `docs/bugs/claude-session-duplicate-split.md` 根因 F（新增）
...
➡️ 详见计划文件 `C:\Users\hanso\.claude\plans\recursive-sniffing-thompson.md`
...
**根源**：双身份模型和 provider index 已经建立，但写入口还没有真正收口。当前仍可能从多条异步路径写入或修补 registry：

- provider transcript scan：从官方 JSONL 扫描重建历史会话。
- `onAgentDone` / `codex-agent-done`：运行结束后补 `cliSessionId/filePath`。
- panel state sync：保存 UI 状态时旁路同步 registry。
- restore / load history：从 panel state 或 registry 恢复 chat 对象。
- repair：启动或加载时清理历史污染。

这些入口如果各自 upsert provider binding，就会出现“index 已指向新 chatKey，但旧 `sessions/<oldChatKey>.json` 还在”的中间态。后续扫描如果绕过 index 或 fallback 全量读 sessions，旧 record 可能再次变成可见会话。
...
近期补丁已经修住“provider merge 后删除 orphan record 文件”并补了回归测试，但这只是止血。T165 Phase 1 已把唯一 owner 契约前移到 `upsertSessionRecord()`：写入时同时检查 `index.json.providers` 和 `sessions/*.json`，同一 provider identity 只保留一个 owner record，并删除所有非 owner orphan 文件；`syncPanelStateSessions()` 显式作为 `providerBindingSource: 'panel'` 调用，只补缺失 provider 字段，不能用旧 panel cache 覆盖已有 active binding。

这仍不是完整端到端收口。后续必须按 T167 补 Electron E2E 覆盖 provider scan / done / restore / 重启顺序，确认主进程 `onAgentDone` 与 renderer restore 在真实 IPC 链路中不会重新制造重复会话。repair 仍只作为历史脏数据兜底，不应成为日常正确性依赖。

**排查入口**：

1. 对同一个 `agent + cliSessionId/filePath`，检查 `index.json.providers` 是否只指向一个 `chatKey`。
2. 再检查 `sessions/*.json` 是否仍有旧 record 持有同一 provider identity。
3. 追踪本次写入来自 scan、done、panel sync、restore 还是 repair。
4. 如果发现同一函数第三次补丁仍在修重复会话，停止点状修复，按 T165 收口 owner API 和契约测试。
5. T165 Phase 1 后，额外确认旧 panel state 重放不会把 `thread-new` 之类 active provider binding 改回 `thread-old`。

➡️ 详见 `docs/plan/2026-06-17-session-registry-and-official-dir-boundary.md`
➡️ 详见 `docs/plan/2026-06-18-agent-session-identity-registry.md`
➡️ 详见 `docs/TODO.md` T165
...
```
1. 是 Claude 还是 Codex？
   ├─ Claude → 读 docs/bugs/claude-session-duplicate-split.md
   └─ Codex  → 读 docs/bugs/codex-conversation-interruption.md
...
| `docs/bugs/claude-session-duplicate-split.md` | Claude 5 个根因（A-E）的详细路径、代码行号、修复路线 |
| `docs/bugs/codex-conversation-interruption.md` | Codex 2 轮根因、run ownership、done 窗口、Opus 复核意见 |
