...
2. 症状分类：
   ├─ 侧栏出现重复/分支的对话
   │   → Trap 2 (异步绑定无匹配) + Trap 1 (Provider 切换)
   │   → 检查 pending chat 数量、扫描时序
   │
   ├─ 对话内容互串/发到错误的对话
...
2. **`/clear` 命令的会话重建** → 需要同时断开 UI chat 与旧 `cliSessionId/filePath`，并在 MindCraft registry 标记旧 provider session `hiddenFromScans`；否则关闭应用重启后，全量扫描官方 JSONL 会把旧 transcript 重新导入侧栏，形成同名重复会话。不要删除或写 sidecar 到 `~/.claude`，隐藏标记必须写在 `{userData}/session-registry/`。
3. **Codex slash 命令对活跃会话的干扰** → `codex-list-slash-commands` 可能对活跃会话 `resumeThread`，触发竞态。（已在 Codex 文档 R2 中提及但未修复。）
