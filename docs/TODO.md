...
| T181 | perf/architecture | **CodeHub Startup / Activation Hot Path 收口**：不再继续零散加缓存；先量化并治理 CodeHub all-panel mount、CodeX 初始化 await active project scan、active chat scan reload 闪烁。streaming assistant `v-html` 优化降级为后续阶段，只处理 active streaming 输入卡。详见 `docs/plan/2026-07-05-hot-path-governance-and-streaming-render.md`。 | P1 | 📝 待 Phase 0 |
...
| 编号 | 说明 | 方案入口 |
|------|------|------|
| T181 | **CodeHub Startup / Activation Hot Path 收口**：旧性能任务已完成各自阶段目标；剩余卡顿先查 CodeHub startup / activation lifecycle，再按数据决定是否去掉 CodeX scan await、修 active chat reload 闪烁，streaming render 只作为后续阶段。 | `docs/plan/2026-07-05-hot-path-governance-and-streaming-render.md` |
