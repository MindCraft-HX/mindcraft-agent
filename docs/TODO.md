...
| T145 | feature | **Token 数据实时增长**：✅ Claude SDK `getTokenMetrics()` inputTokens 改用 `Math.max()` 取流式消息值（不再跳过无 stop_reason 消息）；✅ Codex + ClaudeCode StatusBar 加数字弹跳动画（350ms scale-up）。简易对话留给 T146。详见 `docs/agent-architecture.md` §16。 | P2 | ✅ 已完成 |
...
- 数字动画：CSS `@keyframes sb-count-up`，350ms，`transform: scale(1.15)` + 颜色闪变
- `ov > 0` 守卫防止切 tab / 初始加载时误触发
