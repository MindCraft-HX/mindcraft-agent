...
- 2026-07-05 晚补充：T176 / T177 / T177-P2 / T179 均已完成各自阶段目标；剩余卡顿不再继续归入“加缓存”，统一进入 T181：`docs/plan/2026-07-05-hot-path-governance-and-streaming-render.md`。下一步先治理 active streaming assistant 的 `v-html` 热路径，再视数据评估 activation work queue。
...
| T180 | architecture/metrics | **Metrics context authority + renderer convergence**：收口 `context` 的权威来源、carry-forward 规则和 Claude/CodeX 首次 hydrate 对称性。目标是解决“新回合开始时 context 回退默认值”“首次进入会话 StatusBar 偶发为空”“低置信度 context 覆盖高置信度 context”这三类重复回归。执行入口：`docs/plan/2026-07-05-metrics-context-authority-and-renderer-convergence.md`。 | P1 | 📝 待实施 |
...
| T181 | perf/architecture | **Hot Path Governance and Streaming Render 收口**：不再继续零散加缓存；先修 active streaming assistant 反复 `v-html` 重建整段 DOM，改为 streaming 纯文本、done 后 markdown 一次渲染；若仍有切换后短卡，再做 activation work queue 最小收口。详见 `docs/plan/2026-07-05-hot-path-governance-and-streaming-render.md`。 | P1 | 📝 待实施 |
...
| 编号 | 说明 | 方案入口 |
|------|------|------|
| T181 | **Hot Path Governance and Streaming Render 收口**：旧性能任务已完成各自阶段目标；剩余卡顿统一先修 streaming assistant 的 `v-html` 热路径，随后再按数据评估 activation work queue。 | `docs/plan/2026-07-05-hot-path-governance-and-streaming-render.md` |
...
| 2026-07-05 | perf/architecture | **T181 热路径治理入口登记**：T176/T177/T177-P2/T179 不再继续承载新优化；剩余卡顿不以继续加缓存为目标，先做 streaming assistant plain text，done 后 markdown 一次渲染。 |
