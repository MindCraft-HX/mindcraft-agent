...
| T201 | architecture/storage | **Session / Panel Storage Convergence**：以 T199 审计为输入，收敛 `session-registry`、`*-panel-state.json`、renderer restore persistence 的权威边界。目标是把 session identity/runtime 元数据迁入 SQLite，缩减 panel-state 为 UI 布局层，保留官方 transcript 与运行时文件边界；不迁移消息正文、不重开 provider authority。执行入口：`docs/plan/2026-07-07-T201-session-panel-storage-convergence.md`。 | P1 | 📝 待实施 |
