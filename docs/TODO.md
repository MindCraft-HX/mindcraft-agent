...
| T041 | refactor | Lite 版重构：在共享 Agent 层基础上裁剪宿主能力与导航 | P1 | 可开始细化 |
| T045 | refactor | Agent 抽离：提取 Claude / Codex / codeHub / agentCommon 为共享 `packages/agent` 层 | P1 | Phase A 已完成，进入稳定与复用阶段 |
| T046 | bug | ClaudeCode 会话重复 / 接力问题：重复 jsonl 绑定、active 丢失、renderer session 身份污染 | P0 | 仍复现，专项排查中 |
...
- 关键回归与构建已通过
- 当前阶段不再继续大规模拆分，后续以正常修 Bug 和支撑 Lite 版复用为主

## Lite 版下一步建议

1. 明确 Lite 版保留的 Agent 能力边界。
2. 先做宿主层裁剪设计，不急着继续拆共享层。
3. 只有出现 Full / Lite 明确分叉点时，再追加共享配置抽象。
