...
| T176 | perf/ux | **大 Session 渲染卡顿收口**：Phase 1 已完成（history tool 折叠 + ToolBash 懒挂载，expanded tools 29-43→0-3），仍有冷渲染输入卡顿。Phase 2a-0：renderContent 量化探针先行，数据驱动选方案（computed 缓存/大消息折叠/其他）。详见 `docs/plan/2026-07-04-large-session-rendering-performance.md`。 | P1 | 🔍 Phase 2a-0 |
