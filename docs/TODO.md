...
已单独注册 T165-T168，避免这些问题只在触发 bug 后才被动处理。Claudecode 执行前应先读对应文档，不要把它们并入普通“减行数”重构。
...
| T165 | architecture/bug | **Session Registry ownership 收口**：统一 provider scan / `onAgentDone` / panel sync / restore 的 `chatKey + provider identity` 写入口；panel state 降级为 UI 缓存，不再作为 provider binding 权威；repair 只处理历史脏数据兜底；补 orphan record、跨 project、scan/done 顺序竞态回归测试。详见 `docs/session-pitfalls.md#trap-8session-registry-ownership-中间态`、`docs/plan/2026-06-17-session-registry-and-official-dir-boundary.md`、`docs/plan/2026-06-18-agent-session-identity-registry.md`、`docs/bugs/codex-conversation-interruption.md`。 | P1 | 📝 待方案 |
...
| T167 | test/architecture | **Session/Run 竞态 E2E 前置覆盖**：在继续 R10 或 T165 ownership 收口前，补 Electron E2E/集成测试覆盖 ClaudeCode/CodeX 新建、scan、done、restore、重启后无重复会话，以及 CodeX done 后快速发送下一轮。详见 `docs/session-pitfalls.md`、`docs/plans/remaining-refactoring-roadmap.md#6-验证基线`。 | P2 | 📝 待方案 |
| T168 | refactor/bug | **CodeX event rendering contract 收口**：修复 CodeX tool call 偶发不渲染、assistant 空泡泡、`<thinking>tool call</thinking>` 原始标签泄漏。当前判断为 live stream、history restore、renderer 三套事件解释不一致；目标是保留中间进度可见，同时把 `custom_tool_call` / `function_call` / progress / final assistant 文本归入明确 UI 契约。详见 `docs/plan/2026-07-01-codex-event-rendering-contract.md`。 | P1 | 📝 待实施 |
