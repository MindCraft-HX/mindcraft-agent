...
## 2026-07-03 本地存储重构启动：Provider Storage Migration
...
已注册 T174，执行入口：`docs/plan/2026-07-03-provider-storage-migration.md`。
...
| T174 | architecture/storage | **Provider Storage Migration**：让 SQLite 成为 CodeX / ClaudeCode provider 配置权威源；新增 provider repository、v2 `sort_index`/projection bookkeeping、legacy fallback/backfill/projection；导入/导出/active/reorder 统一走 repository；保留 legacy projection 作为回滚窗口；不迁移 session/panel/transcript。执行交接：`docs/plan/2026-07-03-provider-storage-handoff.md`；方案详见 `docs/plan/2026-07-03-provider-storage-migration.md`。 | P1 | 📝 待实现 |
...
| T177 | perf/diagnostics | **Session 切换后台任务延迟与主线程竞争**：调查 `ensureChatMessagesLoaded.ipc`、`sessionDraft.loadDraftForChat`、`refreshActiveSessionInstructionState`、`refreshMetricsForChat` 的 1s 级 wall time；先对齐 renderer IPC wall time 与 main handler time，再按证据修 draft/instruction/metrics/history apply。详见 `docs/plan/2026-07-04-session-switch-background-task-latency.md`。 | P1 | 🔍 待调查 |
| T179 | perf/architecture | **Project / Session activation work graph 收口**：不再继续零散加缓存，先用 activationId 串起 codeHub → project → session → history/draft/instruction/metrics/scan/registry 全链路；若数据证明 cache hit 仍有 registry 写副作用，再拆 `attachRegistrySessionToScanSummary` 的 upsert 副作用并评估 `upsertSessionRecord` no-op 快速返回。详见 `docs/plan/2026-07-05-project-session-activation-work-graph.md`。 | P1 | 📝 待 Phase 0 |
| T179-P1 | perf/architecture | **scan cache hit 去写副作用**：`attachRegistrySessionToScanSummary()` 名义是合并但内部会 `upsertSessionRecord()`，导致 scan cache hit 仍可能同步读写 registry。Phase 0 量化证明后，拆为 `ensureRegistryFromProviderScan()`（允许写，仅 cache miss/repair/new transcript）和 `mergeRegistryFieldsIntoScanSummary()`（纯读/纯合并，hot cache path 使用），并评估 `upsertSessionRecord()` no-op 快速返回。 | P1 | 📝 已登记，等待 T179 Phase 0 数据 |
...
| 编号 | 说明 | 方案入口 |
|------|------|------|
| T177-P2 | **剩余卡顿归因：Renderer DOM/Layout 或 Vue patch**。先采集 Chrome Performance trace，确认 Scripting / Layout / Paint / DOM node / IPC queue 哪一项是主因，再决定分片挂载、detail 懒挂载、响应式边界或虚拟列表。禁止先写优化代码。 | `docs/plan/2026-07-04-renderer-dom-layout-and-cache-governance.md` |
| T179 | **Project / Session activation work graph 收口**：T178 已做一版 scan 调度/缓存，下一步不继续盲目加缓存，而是量化切换链路并治理 cache hit 的 registry 写副作用。 | `docs/plan/2026-07-05-project-session-activation-work-graph.md` |
