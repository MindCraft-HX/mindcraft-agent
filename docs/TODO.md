...
| T143 | feature | **CodeX 多模型选择器：去硬编码 + 可选模型配置**：SelectModel.vue 去硬编码，改为 open() 动态接收模型列表；ProviderForm 新增备选模型 1/2/3 输入框；codexAgent 新增 `codex-get-alternative-models` IPC；index.vue 构造模型列表传入 SelectModel。详见 `docs/plan/2026-06-21-codex-multi-model-selector.md`。 | P0 | ✅ 已完成 |
