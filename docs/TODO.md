...
| T176 | perf/ux | **大 Session 渲染卡顿收口**：CodeX 大历史 session 冷切入后大量 tool 默认展开并挂完整 bash output，造成 DOM/Layout/Paint/hit-test 过重；Phase 1 只做 history tool 默认折叠 + ToolBash 大输出懒挂载，live 当前回合体验不变。详见 `docs/plan/2026-07-04-large-session-rendering-performance.md`。 | P1 | 📝 待实现 |
