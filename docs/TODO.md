...
- 2026-07-04 晚补充：T177 主线已验收，CodeX + Claude metrics 主进程阻塞已修。剩余体感卡顿转入 T177-P2，只做 renderer Performance trace 归因；执行入口：`docs/plan/2026-07-04-renderer-dom-layout-and-cache-governance.md`。T178 session scan 缓存化暂缓，先按缓存治理规则登记 owner/key/invalidation/limit，不再零散加 Map。
...
| T178 | **Session 扫描缓存化（暂缓）**：只有在 trace 或 perf 数据证明 `handleRefreshSessions` 是当前卡顿主因时再开工。开工前必须按缓存治理规则登记 owner/key/value/invalidation/limit/stale policy，避免继续零散加 Map。 | `docs/plan/2026-07-04-renderer-dom-layout-and-cache-governance.md` §7 |
