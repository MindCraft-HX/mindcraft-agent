...
| T169 | perf/refactor | **Renderer 高频链路性能瘦身**：draft 已迁移到 session-registry 后，继续收口 ProjectTabs / CodeHub summary 过宽依赖、textarea 每键同步 layout、saveHistory 双重 clone。先加开发态探针确认排序，再分 phase 执行；不混入 CodeHub SessionIndex 大重构。详见 `docs/plan/2026-07-02-renderer-hot-path-performance.md`。 | P1 | 📝 方案完成，待执行 |
| T170 | perf/ux | **Session / Tab 切换性能收口**：T169 后输入框不再垮，但点 session、切 CodeHub tab、`Ctrl+Tab` 环绕仍卡。收口 CodeHub tab 激活后的重复 session scan、dev console 噪音、session 切换 scroll 状态恢复；保留 session scan 防御逻辑，不写 scroll 到 session registry。详见 `docs/plan/2026-07-02-session-tab-switch-performance.md`。 | P1 | 📝 方案完成，待执行 |
