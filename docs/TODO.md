...
| T135 | tech-debt | **Session Registry 后续增强**：`writeJsonAtomic()` 仍未做 fsync（当前已有 tmp+rename，Windows 风险较低）；Claude/CodeX session instruction 前端函数仍有重复；CodeX session instruction 注入仍走 user prompt 前缀，后续确认 SDK 是否有更合适的 system/developer instruction 通道后再收敛；`bothFailed` locale key 仍是死代码清理项。 | P3 | ⏸️ 记录 |
