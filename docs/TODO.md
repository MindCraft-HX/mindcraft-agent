...
> 最后更新：2026-06-17（docs 仍为本地知识库；T126 修正覆盖实际用户代码块路径；T125 仍待后续处理）
>
...
| T126 | theme | **用户消息气泡高亮变量完善**：Claude/CodeX 用户消息代码块已补 `--cc-user-hljs-*` token 色 + `--cc-user-link` fallback；token 规则作用于 `.user-msg .code-block .hljs-*` 并使用 `!important` 覆盖父级全局 hljs 规则，确保 fenced code block 的 `.code-card` 路径也生效。 | P3 | ✅ 已完成 |
| T125 | tech-debt | **Sass legacy JS API warning**：尝试在 `vite.config.js` 设置 `css.preprocessorOptions.scss.api: 'modern'` 后，`npm run build` 仍会输出 Dart Sass `legacy-js-api` warning。当前构建成功但 warning 未消除，后续仍需单独定位 Vite/Electron 多配置链路。详见 `# T125 详情`。 | P3 | 🔴 待处理 |
...
| T092 | feature | ~~**Worktree 多分支隔离**：每个 session 绑定独立 git worktree。~~ **决定不做了**：SDK 不原生支持，App 层面实现 = 给 session 开个新目录 = 跟新建项目区别不大；单窗口多 tab 架构下成本远超收益。forkSession (T090) 更实用。 | P1 | ❌ 暂不处理 |
