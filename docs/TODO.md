...
> 最后更新：2026-06-17（docs 重新纳入 git；新增 T134 Session Registry 与官方目录边界重构；T125 已静音，根治待升级 Vite）
>
...
| 编号 | 分类 | 说明 | 优先级 | 状态 |
|------|------|------|:------:|------|
| T134 | refactor | **Session Registry 与官方 Agent 目录边界重构**：MindCraft 自有数据禁止污染 `~/.claude` / `~/.codex`；建立 `{userData}/session-registry/` 保存 chat/session title、description、instruction、模型/effort、chatKey↔cliSessionId 映射和编排元数据。官方 JSONL transcript 不迁移；CodeX panel state 已迁到 `{userData}/codex-panel-state.json`（读旧写新，不删旧文件）；Session Registry 已建立并旁路同步 Claude/CodeX panel state 映射；Claude per-session model/effort 新写入已迁到 registry，旧 `.meta.json` 只读 fallback；Session Instruction 文本注入已接入 Claude/CodeX，附件读取暂缓；诊断日志、临时上传和 skills catalog cache 已迁到 `{userData}`。详见 `docs/plan/2026-06-17-session-registry-and-official-dir-boundary.md`。 | P1 | ✅ Phase 5 完成 |
