...
**遗留 & 待办**
- R04（Tab/History composable 收敛）：顺延，setupClaudeHandlers/setupCodexSdkHandlers 各自 ~2400-2900 行仍是最大瓶颈
- R06（Renderer Convergence 剩余收口）：待执行
- R07（main.js 更深拆分）：contextMenu、scattered IPC handlers 仍可提取
- `setupCodexSdkHandlers()` (~2463 行) 和 `setupClaudeHandlers()` (~2963 行) 过于耦合，需专项设计拆分方案
