...
- Claude assistant/result `usage` normalizes current-turn tokens as `in = input_tokens + cache_creation_input_tokens`, `cache = cache_read_input_tokens`, `out = output_tokens`; when token fields exist, the same final usage must also emit a single-sample context estimate (`in + cache + out`) so stale compact/old context values cannot survive beside a new high-cache final turn.
