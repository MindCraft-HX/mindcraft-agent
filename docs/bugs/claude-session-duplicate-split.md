...
### 🟡 根因 F：`selectDir` 重建 chat 时漏传 `_userRenamed`

**文件**：`packages/agent/src/components/claudeCode/index.vue:1958-1974`

**问题本质**：`loadProjectSessions(dir)` 返回 `_userRenamed: Boolean(s.isCustomTitle)`（从 JSONL 提取的 custom-title），但 `selectDir` 构建 chat 对象时没有传递这个字段。chat 对象的 `_userRenamed` 为 `undefined`（falsy）→ 下次扫描 `if (!cached._userRenamed)` → `true` → 覆盖自定义命名。

**触发条件**：关闭项目 Tab 后重新打开，用户选择同一个工作目录。

**典型路径**：
1. 用户给会话自定义命名 → SDK 写 `custom-title` 到 JSONL → `chat._userRenamed = true`
2. 关闭项目 Tab → `deleteProject` → 项目从 `panel-state.json` 移除
3. 重新打开 → `onAgentSelected` → `createProject` → 空项目
4. 选择目录 → `selectDir(dir)` → `loadProjectSessions(dir)` 扫描 JSONL → 返回 `_userRenamed: true` ✅
5. `selectDir` 构建 chat 对象 → **漏传 `_userRenamed`** ❌ → `_userRenamed = undefined`
6. `saveHistory()` → panel-state.json 保存 `_userRenamed: false`
7. 后台扫描 → `if (!cached._userRenamed)` → `true` → 名称被覆盖为默认

**为什么修了 7+ 轮都没成功**：之前的修复全集中在"扫描防覆盖"下游，但 `selectDir` 重建 chat 时从未设置 `_userRenamed`。上游丢了数据，下游怎么防都没用。

**修复**：`selectDir` 加一行 `_userRenamed: Boolean(s._userRenamed)`（2026-06-11 已修复）。

**教训**：当数据流中有"完全重建"步骤，必须确保重建产物包含所有 UI 状态字段。详见 `docs/session-pitfalls.md` Trap 6。
...
| **P2** | **F: selectDir 漏传 `_userRenamed`** | `selectDir` 构建 chat 时加 `_userRenamed: Boolean(s._userRenamed)`（1 行） | `index.vue` |
...
### ⚠️ P0-A 回归：TDZ 错误 — `sender` before initialization (2026-06-11)

**问题**：P0-A 修复在 `for await` 循环中新增 `safeSend(sender, 'claude-agent-early-cli-session', ...)` 调用时，**未将 `const sender = ...` 声明同步上移**。新调用位于声明之前（TDZ 内），导致首次对话抛出 `ReferenceError: Cannot access 'sender' before initialization`。

**为什么首次触发、后续没事**：首次消息到达时 `cliSessionIds` 为空 → 进入 `if (!cliSessionIds.has(sessionId))` 分支 → 命中 TDZ 抛出异常。后续消息 `cliSessionIds` 已注册 → 跳过该分支 → 不触发。

**涉及代码**（`claudeAgent.js:2155-2168`）：
...
for await (const msg of q) {
    ...
    gotAnyMessage = true
    // 新增的 safeSend 调用在 const sender 声明之前！
    if (msg?.session_id) {
    ...
    const sender = agentSessions.get(sessionId)?.event?.sender || event.sender  // 声明在此

// ✅ 修复后 — const sender 提前到首次使用之前
for await (const msg of q) {
    if (!agentSessions.has(sessionId)) break
    const sender = agentSessions.get(sessionId)?.event?.sender || event.sender  // 声明在上
    gotAnyMessage = true
    // 后续所有使用均在声明之后 ✓
```

**修复**（1 处）：将 `const sender` 声明从循环体底部移至 `gotAnyMessage = true` 之前。

**教训**：
- 在已有代码块中**展开单行为多行 block 并新增变量引用时**，必须检查该变量在当前作用域内的声明位置
- `const`/`let` 的 TDZ 需要**特定代码路径**才能触发（此处是 `!cliSessionIds.has(sessionId)`），常规测试不一定覆盖
- 补丁叠加时优先考虑"这个新调用依赖的变量在哪声明的"——声明可能在不显眼的位置
...
### P2-F: selectDir 漏传 `_userRenamed` ✅

**问题**：关闭项目 Tab 后重新打开，`selectDir` 重建 chat 对象时遗漏 `_userRenamed` 字段 → 自定义命名丢失。

**修复（1 处）**：

| # | 文件 | 位置 | 改动 |
|---|------|------|------|
| 1 | `index.vue` | `selectDir` L1972 | chat 构建对象加 `_userRenamed: Boolean(s._userRenamed)` |

**教训**：这是 Trap 6 的经典案例——数据流中有"完全重建"步骤时，下游防护再多也无效，必须在上游补字段。
