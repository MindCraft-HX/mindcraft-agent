...
`contextUsage` 的表格口径只允许用于明确的 session context 样本（例如 `system context_usage`、`token_count info`、compact 边界、Claude assistant usage 作为 session context sample）。Claude transcript 的 compact 边界以真实 JSONL 字段 `system subtype=compact_boundary + compactMetadata.preTokens/postTokens` 为准；优先使用 `postTokens` 作为压缩后的当前 context。其后如继续有 Claude assistant usage，应允许它把 `contextUsage` 推进到最新 session 占用，但这个更新只能作用于 `contextUsage/contextWindow`，不能反向污染当前回合 `in/out/cache`。第三方 Claude SDK provider 的 assistant usage 里可能包含大量 cache read，因此必须把“session context 更新”和“current-turn token 归属”分开处理。
...
运行中切换 CodeX 会话时，`StatusBarMetrics` 必须按 active tab 身份、`sessionId/cliSessionId`、`thinking/_thinkingStart` 主动同步 timer 并触发一次 metrics refresh；不能只依赖新流式事件，否则切回正在运行的会话会短暂只显示模型/context 旧快照。
