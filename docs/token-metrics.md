...
`contextUsage` 的表格口径只允许用于明确的 session context 样本（例如 `system context_usage`、`token_count info`、compact 边界）。Claude transcript 的 compact 边界以真实 JSONL 字段 `system subtype=compact_boundary + compactMetadata.preTokens/postTokens` 为准；优先使用 `postTokens` 作为压缩后的当前 context。Claude assistant/result `usage` 只能用于本回合 `in/out/cache`，不能推进 `contextUsage/contextWindow`；否则大量 cache read 会污染压缩入口和 StatusBar context。
