...
HTML 不再只显示预览，也不照搬 Markdown 的源码/预览分屏。HTML 预览是一个完整页面，窄幅分屏会同时损害源码和页面布局，因此只保留两种模式：
...
- `预览`：在非同源 sandbox iframe 中渲染；允许页面内联脚本完成按钮、菜单、Tab 和动画等本地交互，但不授予同源、宿主 API、弹窗、下载或顶层导航能力。注入的 preview CSP 默认阻断外部网络、嵌套 frame、worker、object 和表单提交。
- HTML 不提供文档内部的源码/预览分屏；需要对照时用主工作区把 HTML 文档与其他内容并排，或在源码/预览间切换。
...
6. HTML 文档预览使用仅含 `allow-scripts`、不含 `allow-same-origin` 的 sandbox iframe；`srcdoc` 在所有用户内容前注入默认 `default-src 'none'` 的独立 CSP，只放行内联脚本/样式与必要的 data/blob 静态资源，阻断 connect、frame、worker、object、base 和 form action。顶层导航、弹窗和下载全部禁止。HTML Preview 不得拿到 preload、host origin storage 或 terminal bridge；需要联网或完整浏览器能力时必须交给系统浏览器，不扩权内嵌预览。
...
- 加入 document file signature/watcher 和 dirty external-change conflict；HTML 接入 `源码（默认）/ 隔离交互预览` 两种模式，不提供文档内部的源码/预览分屏。
