...
### Phase 2 — ClaudeCode 对齐（已跳过）

> **2026-07-02 决策：跳过。**
> 
> T171 cache-first + dedup 已让 ClaudeCode `refreshMetricsForChat` avg=0.10ms。
> 合并 `getTokenMetrics` + `getSpeedMetrics` 只省一次冷读文件遍历，用户感知不到。
> 收益 / 风险比不成立，不动。
...
- [x] 快速切换 10 次 tab，UI 不冻结
- [x] 大 session 点击后立即显示缓存或空状态，不等待 metrics
- [x] `switchChat` 同步段仍为 ms 级
- [x] `codex-metrics.session-read` 不阻塞切换第一帧
- [x] StatusBar 不串 session、不残留旧 session 指标
- [x] scroll 恢复正常
- [x] `npm test` 通过（311 pass）
- [x] `npm run build` 通过
...
| 文件 | 改动 |
|---|---|
| `packages/agent/src/components/codeX/index.vue` | `refreshMetricsForChat` 按 reason 分流：interaction 异步化 |
| `packages/agent/electron/codexAgent.js` | `collectMessage` `console.warn` → `if (CODEX_DEBUG)` |

> Phase 2（ClaudeCode 合并遍历 + 后台化）已跳过。见 Phase 2 节决策。
