...
> 日期：2026-07-04
> 状态：T177-P2 Phase 1 已实施；剩余 project/session activation 卡顿转入 T179
...
用户仍感到“还有点卡”。下一步不能继续把 T176-P2、T177-P2、T178 三线并行推进。当前分成两条已收口主线：

- **T177-P2：Renderer DOM/Layout/Paint。** Performance trace 已确认 2.3s/1.2s long task 来自一次性挂载 60 条消息及大量 tool/detail DOM；Phase 1 已落地分片挂载。
- **T179：Project / Session activation work graph。** 如果项目 tab 切换后仍卡，下一步不再继续盲目加缓存，而是量化 codeHub → project → session → history/draft/instruction/metrics/scan/registry 的触发图。

T178 session scan 缓存化已实施一版；后续治理入口是 `docs/plan/2026-07-05-project-session-activation-work-graph.md`，重点是 cache hit 路径不能继续隐式 registry upsert。

## 2. 当前判断

Renderer DOM 方向已经确认过一次：
...
Phase 1 已采用分片挂载降低一次性 DOM/Layout/Paint 压力。若后续仍有输入卡顿，继续用本文件的判定树处理 DOM/Vue 问题；若卡顿集中在跨 project tab 切换，转 T179。
...
T178 session scan 缓存化已实施一版。后续不再扩大零散 Map，而是按 T179 检查：

- cache hit 是否仍触发 registry 写。
- scan wall / scan apply / registry attach / upsert 各自耗时。
- project/session activation 同步段是否仍被后台 scan 抢占。
...
```text
如果问题是大 session 内输入/滚动仍卡，继续走 T177-P2：采集并分析 renderer Performance trace。
...
如果问题是跨 project tab 切换后卡，转 T179：
