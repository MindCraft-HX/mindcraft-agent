            ...
            <div class="chat-message is-profile" v-if="item.profile">
              <div class="profile-title">
                ...
                <template v-if="!item.showMore">
                  ...
                  <span v-else>点击展开查看角色【{{ item.profile.name || "无" }}】的信息</span>
                </template>
              </div>
              <template v-if="item.showMore">
                <div class="profile-content">昵称: {{ item.profile.name || "无" }}</div>
                <div class="profile-content">角色介绍： {{ item.profile.character_description || "无" }}</div>
                <div class="profile-content">性格: {{ item.profile?.personality?.join("；") || "无" }}</div>
                <div class="profile-content">沟通风格： {{ item.profile?.tone || "无" }}</div>
                <div class="profile-content">台词： {{ item.profile?.lines?.join("；") || "无" }}</div>
              </template>
...
mitt.off("createCharacterInput")
mitt.on("createCharacterInput", (data) => {
...
const stopMicrophone = async () => { 
  ...
  messageAppend()
  character_ws.sendMessage('createCharacter', { asr_text: '', character_profile: character.value })
  sendVoice(buffer)
