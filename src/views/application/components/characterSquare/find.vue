<template>
  <div class="find">
    <div class="title">
      <el-button style="margin-right: 12px;" type="primary" plain @click="returnList" v-if="showCharacterDetail">
    ...
    <characterInfo v-model:character="characterDetail" v-if="showCharacterDetail">
      <template v-slot:btn>
        <el-button style="margin: 5px;" type="primary" plain @click="editCharacter" v-if="characterDetail.is_owner">
          <div class="mindcraft-flow-win-iconfont icon-mindcraft-bianji"></div>编辑</el-button>
        <el-button style="margin: 5px;" type="primary" plain @click="confirmCopy">
          <div class="mindcraft-flow-win-iconfont icon-mindcraft-fuzhi1"></div>复制
        </el-button>
        <el-button style="margin: 5px;" type="primary" plain @click="shareCharacter" v-if="characterDetail.is_owner">
          <div class="mindcraft-flow-win-iconfont icon-mindcraft-fenxiang"></div>{{ !characterDetail.is_share ? '分享' : '取消分享'}}</el-button>
        <el-button style="margin: 5px;" type="primary" plain @click="deleteCharacter" v-if="characterDetail.is_owner">
...
const pageName = computed(() => {
  return showCharacterDetail.value ? '角色详情' : '角色广场'
})
...
const confirmCopy = () => {
  if(characterDetail.value?.is_owner) {
    copyCharacter()
  } else {
    ElMessageBox.confirm("复制角色将消耗10,000积分", "复制角色积分消耗", {
...
const copyCharacter = async () => {
  ...
  try {
    ...
    notice?.close()
    ElMessage.error(JSON.parse(error?.response?.data || "{}")?.message || "复制角色失败")
  }
}
