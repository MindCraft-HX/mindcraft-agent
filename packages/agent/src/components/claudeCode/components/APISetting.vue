    ...
    <div v-if="showImportPreview" class="settings-overlay" @click.self="cancelImportPreview">
      <div class="import-dialog import-preview-dialog">
        <div class="import-dialog-header">
          <span>{{ $t('settings.importPreviewTitle', { source: $t('settings.importLocalClaudeConfig') }) }}</span>
...
async function showImportConfigDialog() {
  ...
  const ok = await confirmDialogRef.value?.open({
    ...
    okText: t('settings.importLocalClaudeConfig'),
    cancelText: t('common.cancel'),
  })
  if (!ok) return

  try {
    const preview = await window.electronAPI?.claudeConfigImportPreview?.({ source: 'local-cli' })
    if (!preview?.ok) {
...
async function commitImport() {
  ...
  try {
    const result = await window.electronAPI?.claudeConfigImportCommit?.({
      source: 'local-cli',
      providers: decisions,
    })
...
.import-dialog {
  ...
  border: 1px solid var(--cc-border);
  border-radius: 10px; box-shadow: 0 20px 60px var(--cc-shadow);
